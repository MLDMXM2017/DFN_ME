...
class GCForest(object):
    ...
    def transform(self, X):
        """
        return:
            if only finegrained proviede: return the result of Finegrained,细粒度扫描
            if cascade is provided: return N x (n_trees in each layer * n_classes),级联森林
        """
        if self.fg is not None:
            X = self.fg.transform(X)
