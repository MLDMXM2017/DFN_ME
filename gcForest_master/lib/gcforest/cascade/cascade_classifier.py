...
class CascadeClassifier(object):
    ...
    def fit_transform(self, X_groups_train, y_train, X_groups_test, y_test, stop_by_test=False, train_config=None):
        ...
        LOGGER.info("X_groups_train.shape={},y_train.shape={},X_groups_test.shape={},y_test.shape={}".format(
            ...
            [xt.shape for xt in X_groups_test] if is_eval_test else "no_test",
            y_test.shape if is_eval_test else "no_test"))
        ...
        assert n_classes == len(np.unique(y_train)), "n_classes({}) != len(unique(y)) {}".format(n_classes,
                                                                                                 np.unique(y_train))
        ...
        try:
            ...
            while 1:
                ...
                train_avg_acc = calc_accuracy(y_train, np.argmax(y_train_proba_li, axis=1),
                                              'layer_{} - train.classifier_average'.format(layer_id))
                train_acc_list.append(train_avg_acc)
                if n_tests > 0:
                    y_test_proba_li /= len(self.est_configs)
                    test_avg_acc = calc_accuracy(y_test, np.argmax(y_test_proba_li, axis=1),
                ...
                if self.early_stopping_rounds > 0 and layer_id - opt_layer_id >= self.early_stopping_rounds:
                    ...
                    LOGGER.info(
                        "[Result][Optimal Level Detected] opt_layer_num={}, accuracy_train={:.2f}%, accuracy_test={:.2f}%".format(
            ...
            LOGGER.info(
                "[Result][Reach Max Layer] opt_layer_num={}, accuracy_train={:.2f}%, accuracy_test={:.2f}%".format(
                    opt_layer_id + 1, train_acc_list[opt_layer_id], test_acc_list[opt_layer_id]))
            if data_save_dir is not None:
    ...
    def transform(self, X_groups_test):
        ...
        for layer_id in range(self.opt_layer_num):
            ...
            for ei, est_config in enumerate(self.est_configs):
                ...
                if est is None:
                    raise ValueError(
                        "model (li={}, ei={}) not present, maybe you should set keep_model_in_mem to True".format(
                            layer_id, ei))
                y_probas = est.predict_proba(X_cur_test)
                X_proba_test[:, ei * n_classes:ei * n_classes + n_classes] = y_probas
            self.layer_features.append(X_proba_test)
        LOGGER.info('total leyer feature shape is {}'.format(np.array(self.layer_features).shape))
        return X_proba_test, self.layer_features

    def predict_proba(self, X):
        # n x (n_est*n_classes)
        y_proba, _ = self.transform(X)
    ...
    def save_data(self, data_save_dir, layer_id, X_train, y_train, X_test, y_test):
        for pi, phase in enumerate(["train", "test"]):
            ...
            LOGGER.info(
                "Saving Data in {} ... X.shape={}, y.shape={}".format(data_path, data["X"].shape, data["y"].shape))
